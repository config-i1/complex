...
#' split the real and imaginary parts. See details for explanation.
#'
...
complex2mat <- function(x){
    ...
    for(i in 1:ncolX){
        complexMatrix[1:nrowX*2-1,i*2-1] <- Re(x[,i]);
        complexMatrix[1:nrowX*2,i*2] <- Re(x[,i]);
        complexMatrix[1:nrowX*2-1,i*2] <- -Im(x[,i]);
        complexMatrix[1:nrowX*2,i*2-1] <- Im(x[,i]);
    }
...
complex2vec <- function(x){
    ...
    if(is.matrix(x)){
        ...
        rownamesX <- rownames(x);
        if(!is.null(rownamesX)){
            rownamesX <- paste0(rownames(x), c("_r","_i"));
        }
        colnamesX <- paste0(rep(colnames(x),each=2), c("_r","_i"));
    }
    else{
        ...
        rownamesX <- names(x);
        if(!is.null(rownamesX)){
            rownamesX <- paste0(names(x), c("_r","_i"));
        }
        colnamesX <- c("x_r", "x_i");
        x <- as.matrix(x);
    }
    ...
    for(i in 1:ncolX){
        complexMatrix[,i*2-1] <- Re(x[,i]);
        complexMatrix[,i*2] <- Im(x[,i]);
    }
