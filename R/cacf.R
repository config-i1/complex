...
#'
#' For \code{kind="correlation"} and \code{"covariance"}, the estimates are based
...
#' @param kind character string giving the kind of CACF to be computed. Allowed values
...
#' \item \code{type} The type of moment used in calculation (same as the type argument).
#' \item \code{kind} The kind of correlation (same as the kind argument).
...
#' @export
cacf <- function(x, lag.max=NULL, type=c("direct","conjugate"),
                 kind=c("correlation","covariance"),
                 plot=TRUE){
    ...
    for(i in 1:lag.max){
        xACF[i] <- switch(kind,
                          "covariance"=ccov(xLagged[,1], xLagged[,i+1], type=type, na.rm=TRUE),
                          "correlation"=ccor(xLagged[,1], xLagged[,i+1], type=type, na.rm=TRUE));
    }

    acf.out <- structure(list(acf=xACF, type=type, kind=kind, n.used=obs,
...
#' @rdname CACF
#' @export
pcacf <- function(x, lag.max=NULL, type=c("direct","conjugate"),
                  plot=TRUE){
    # Function is based on acf() from stats
    ...
    acf.out <- structure(list(acf=xPACF, type=type, kind="partial", n.used=obs,
                              lag=c(1:lag.max), series=series), class="cacf");
